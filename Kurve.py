...
class Curve():
        ...
        for sec in self.sections:  # nur polynome berechnen
            match sec.rule:
                ...
                case "Poly 5":
                    ...
                    start_conditions = [self.v[int(sec.t_s*self.scale)-1], self.a[int(
                        sec.t_s*self.scale)-1], self.v[int(sec.t_e*self.scale)], self.a[int(sec.t_e*self.scale)]]
                ...
                case "poly 5 connector":
                    ...
                    start_conditions = [self.v[int(sec.t_s*self.scale)], self.a[int(
                        sec.t_s*self.scale)], self.v[int(sec.t_e*self.scale)-1], self.a[int(sec.t_e*self.scale)-1]]
                    for i, val in enumerate(start_conditions):
                        if val < 0.01 and val > -0.01:
                            start_conditions[i] = 0
    ...
    def update_section(self, this_section, new_t_end, new_p_end, new_rule):
        ...
        if len(self.sections) == this_section.pos:
            self.sections[0].t_s = new_t_end
            self.sections[0].p_s = new_p_end
        else:    
    ...
    def create_section(self, t_start, t_end, p_start, p_end, position_in_curve, rule) -> Section:
        new_section = Section(t_start, t_end, p_start, p_end,
                              position_in_curve, rule, self.zyklus, self.scale)
        return new_section
